...
from ops import lrelu, conv2d, fully_connect, upscale, Pixl_Norm, downscale2d, MinibatchstateConcat, batch_normal
...
class PGGAN(object):
    ...
    def __init__(self, batch_size, max_iters, model_path, read_model_path, data, sample_size, sample_path, log_dir,
        ...
        self.scale_z = tf.placeholder(tf.float32, [self.batch_size, self.sample_size])
    ...
    def build_model_PGGan(self):
        ...
        self.De_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.De_res, labels=self.scale_z))
        self.G_loss = -tf.reduce_mean(self.G_pro_logits)-0.1*self.De_loss
        #
        ...
        #
        self.de_vars = [var for var in t_vars if 'de' in var.name]

        total_para1 = 0
        for variable in self.de_vars:
            shape = variable.get_shape()
            print(variable.name, shape)
            variable_para = 1
            for dim in shape:
        ...
        self.de_vars_n = [var for var in self.de_vars if 'de_n' in var.name]
        ...
        self.de_vars_n_read = [var for var in self.de_vars_n if '{}'.format(self.output_size) not in var.name]
        ...
        self.de_vars_n_2 = [var for var in self.de_vars if 'de_y_rgb_conv' in var.name]
        ...
        self.de_vars_n_2_rgb = [var for var in self.de_vars_n_2 if '{}'.format(self.output_size) not in var.name]
        ...
        self.saver = tf.train.Saver(self.d_vars + self.g_vars+self.de_vars)
        self.r_saver = tf.train.Saver(self.d_vars_n_read + self.g_vars_n_read+self.de_vars_n_read)

        if len(self.d_vars_n_2_rgb + self.g_vars_n_2_rgb+self.de_vars_n_2_rgb):
            self.rgb_saver = tf.train.Saver(self.d_vars_n_2_rgb + self.g_vars_n_2_rgb+self.de_vars_n_2_rgb)
    ...
    def train(self):
        ...
        acc = tf.reduce_mean(tf.cast(tf.equal(tmp,self.scale_z),tf.float32))
        ...
        with tf.Session(config=config) as sess:
            ...
            while step <= self.max_iters:
                ...
                for i in range(n_critic):
                    ...
                    sc_z = (sample_z>0).astype(np.float32)
                ...
                # optimization G
                sess.run(opti_G, feed_dict={self.z: sample_z, self.scale_z: sc_z})
                sess.run(opti_De, feed_dict={self.z: sample_z, self.scale_z: sc_z})
                summary_str = sess.run(summary_op, feed_dict={self.images: realbatch_array, self.z: sample_z, self.scale_z:sc_z})
                ...
                if step % 400 == 0:
                    D_loss, G_loss, De_loss, D_origin_loss, accuracy, alpha_tra = sess.run([self.D_loss, self.G_loss, self.De_loss, self.D_origin_loss, acc,self.alpha_tra], feed_dict={self.images: realbatch_array, self.z: sample_z, self.scale_z:sc_z})

                    print("PG %d, step %d: D loss=%.7f G loss=%.7f, De loss=%.7f, D_or loss=%.7f, acc=%.7f, opt_alpha_tra=%.7f" % (self.pg, step, D_loss, G_loss, De_loss, D_origin_loss, accuracy, alpha_tra))
